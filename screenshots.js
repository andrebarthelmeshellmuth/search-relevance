const PACKAGES = [
  ...
  },
  {
    ...
    screenshots: [
      ...
      { title: "Compare BM25 across tokens", description: "Two products pinned open side by side for the same query, \"office chair\" — the left card's BM25 breakdown is expanded for the token \"office\", the right card's for \"chair\". Their inverse document frequency tells the real story neither product's headline score reveals alone: idf 0.326 for \"office\" (it appears in nearly every product in this catalog) against idf 3.013 for \"chair\" (comparatively rare) — the same query-time boost and a similar term frequency, but \"chair\" is worth roughly 9× more per occurrence simply because it's more specific. Pinning turns the overlay from a per-product tooltip into a real side-by-side comparison tool.", src: "screenshots/search-debug/bm25-compare.png" },
      ...
      { title: "Full German analyzer pipeline (index time)", description: "The richest path this catalog's German analyzer chain produces: \"Bürostuhl\" (office chair), found inside a product description while searching \"stuhl\". Eight stages run in sequence — a char filter that protects the real brand name \"Brennenstuhl\" from being mistaken for \"Stuhl\" further down the chain, the standard tokenizer, lowercasing, german_normalization folding \"ü\" to \"u\", a German stopword filter, a dictionary decompounder splitting the compound into \"büro\" + \"stuhl\", a light German stemmer, and finally the edge-ngram filter used for prefix matching — each stage's own real, live filter definition shown inline, not just its name.", src: "screenshots/search-debug/token-analysis.png" },
